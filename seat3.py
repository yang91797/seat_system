...
class Order(object):
    ...
    def __init__(self):
        self.proxy = None  # {'http': 'http://183.232.113.51:80'}
    ...
    def vie_seat(self):
        ...
        if dev_filter['msg'] == 'ok':
            ...
            title = title.json()
            seat = {'devId': dev_id, 'labId': title['data'][0]['labId'], 'kindId': title['data'][0]['kindId'],
                    'title': title['data'][0]['devName']}
...
def foo(obj, u):
    try:
        ...
        if obj.number < 10:
            ...
            obj.send_email(u['email'], '%s同学你好：图书馆系统故障，未能成功选座' % u['name'])
