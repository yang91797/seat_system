...
class Order(object):
    ...
    def __init__(self):
        self.proxy = None  # {'http': 'http://163.204.246.204:9999', 'https': 'https://163.204.246.204:9999'}
    ...
    def top_speed_seat(self, timeIndex):
        """
        极速抢座
        :return:
        """
        ...
        for i in self.seat_list:
            ...
            while self.flag:
                ...
                if now.hour >= self.h and now.minute >= self.m or now.hour > self.h:
                    result = self.get_seat(seat_msg, timeIndex)
                    if result:
                        return 'ok'

    def vie_seat(self, timeIndex):
        """
        快速抢座
        :return:
        """
        ...
        for i in seat_info:
            ...
            if self.get_seat(seat, timeIndex):
                return True
    ...
    def inquire(self, start, end, index):
        """
        查询是否预约成功
        :return:
        """
        ...
        if response.status_code == 200:
            for item in response.json().get("data"):
                ...
                if item.get("start") == start and item.get("end") == end:
                    ...
                    setattr(self, "seatFlag%s" % index, True)
                    print(eval("self.seatFlag%s" % index), index)
                    info = '%s同学你好：%s : %s' % (self.username, item.get("devName"), time)
                    print(info)
                    self.msg(info)
                    # self.send_email(self.email, info)
                    # self.change(times=True)

    def get_seat(self, seat, timeIndex):
        response = requests.get(
            url='http://seat.hhit.edu.cn/ClientWeb/pro/ajax/reserve.aspx',
            timeout=self.timeout,
            params={
        ...
        time = str(datetime.datetime.now())
        self.g.append(gevent.spawn(self.inquire, self.start, self.end, timeIndex))

        try:
            seatFlag = eval("self.seatFlag%s" % timeIndex)
        except Exception:
            seatFlag = None
        print(seatFlag, '**********************', timeIndex)
...
def auth(func):
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            info = '%s同学%s出现错误：%s，%s' % (u['name'], func.__name__, e, str(datetime.datetime.now()))
            obj.g.append(gevent.spawn(obj.msg, info))
            print(info)
            obj.for_func(func, info)
...
def foo(obj, u):
    # obj.get_proxy()
    for timeIndex, item in enumerate(times(u)):
        obj.startTime = item[0]
        obj.endTime = item[1]
        obj.start = obj.date + ' %s' % item[0]
        obj.end = obj.date + ' %s' % item[1]

        # res_top_speed = obj.top_speed_seat(timeIndex)
        # if res_top_speed:
        #     print('极速抢座成功')
        ...
        for seat in seats:
            if obj.get_seat(seat, timeIndex):
                break
