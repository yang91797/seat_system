    ...
    def top_speed_seat(self):
        ...
        for i in self.seat_list:
            ...
            while self.flag:
                ...
                if self.while_times == 0:
                    time.sleep(3)
                    self.while_times += 1
                if now.hour >= self.h and now.minute >= self.m or now.hour >= self.h and now.minute >= 29 and now.second >= 54 or now.hour > self.h:
...
def foo(obj, u):
    try:
        ...
        for item in seat_time:
            ...
            #     index += 1
            #     print(index)
            #     if index >= len(seat_time):
            #         obj.top = False
            #     if res_top_speed:
            #         print('极速抢座成功')
            #         print(time.perf_counter() - st)
            #         continue
# 311, 312, 317, 318, 300, 306, 294, 288, 335, 321, 322,323
            # # 快速抢座
            # result = obj.vie_seat()
            # if result:
            #     print('快速抢座成功')
        ...
        info = '%s同学链接出现故障:%s，%s' % (u['name'], e, str(datetime.datetime.now()))
        obj.g.append(gevent.spawn(obj.msg, info))
        print(info)
        obj.for_func(foo, info)
    except Exception as e:
